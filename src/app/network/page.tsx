...
const statusStyle = {
  ...
  monitoring: "text-[var(--gold)] bg-[var(--gold-dim)] border-[var(--gold)]/30",
  disrupted:
...
export default function NetworkPage() {
  return (
    <AppShell
      ...
      subtitle="Cluster victims, mules, devices and compounds into actionable, auditable packages"
    >
      <div className="mb-4 flex flex-wrap items-center gap-3 text-sm text-[var(--muted)]">
        <Network
          className="h-4 w-4 text-[var(--phosphor)]"
          aria-hidden="true"
        />
        Graph AI agent. Click nodes to inspect linkages. Demo cluster: Compound
        Alpha
      </div>
      ...
      <div className="mt-6">
        <div className="flex items-center gap-2 mb-3">
          <FileStack className="h-4 w-4 text-[var(--phosphor)]" />
          ...
          {fraudClusters.map((c) => (
            <div key={c.id} className="card p-5 card-hover">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <span className={`badge border ${statusStyle[c.status]}`}>
                  {c.status}
                </span>
              </div>
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="flex justify-between gap-2 pt-1">
                  ...
                  <span className="text-right text-[var(--phosphor)]">
                    {c.leadAgency}
                  </span>
                </div>
              </div>
